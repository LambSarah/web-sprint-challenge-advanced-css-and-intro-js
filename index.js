...
function getArtistByIndex(artists, desiredIndex) {
    return `the artist at index ${desiredIndex} is ${artists[desiredIndex].name}`
}
...
function get20s(artists) {
    ...
    for (let artist in artists) {
        if (artists[artist].years.startsWith("19")) {
            twentys.push(artists[artist].name);
        }
    }
    return twentys;
}
...
function removeArtist(artists, desiredIndex) {
    let newArray = artists.splice(desiredIndex, 1);
    let newArray2 = Array.from(artists);
...
function addArtist(artists) {
    artists.push({
        id: 20,
        name: "Sarah Guidry",
        years: "1984 - February 5, 2021",
        genre: "Web Design",
        nationality: "American",
        bio: "Sarah was born and raised in Southeast Louisiana. At age 36 she began learning at Lambda School.",
    });
    return artists;
}
...
function lotsOfArt(artists) {
    ...
    for (let artist in artists) {
        if (parseInt(artists[artist].paintings) >= 100) {
            over100Paintings.push(artists[artist].name);
        }
    }
